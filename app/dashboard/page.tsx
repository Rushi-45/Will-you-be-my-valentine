...
import { Heart, Sparkles, Bookmark, ArrowRight, Shield } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const [cards, admin] = await Promise.all([listUserCards(), isAdmin()]);
    ...
    <main className="relative min-h-screen bg-stone-50 pt-24 pb-16 text-stone-800 dark:bg-slate-950 dark:text-slate-100">
      ...
      <div className="relative z-10 mx-auto flex max-w-3xl flex-col gap-8 px-4 sm:px-6">
        ...
        {admin && (
          <Link
            href="/admin"
            className="group inline-flex w-fit items-center gap-2 rounded-full border border-amber-200/70 bg-amber-50/80 px-4 py-2 text-xs font-semibold text-amber-700 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-300"
          >
