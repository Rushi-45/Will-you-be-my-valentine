...
type Props = { searchParams: Promise<{ name?: string; year?: string }> | { name?: string; year?: string } };
...
export default async function Image({ searchParams }: Props) {
  ...
  const sub = validYear ? `Class of ${validYear}` : "You made it!";
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <div style={{ fontSize: 62, fontWeight: 900, color: "#fff", textAlign: "center", lineHeight: 1.1, marginBottom: 12, letterSpacing: "-0.03em", textShadow: "0 2px 12px rgba(0,0,0,0.15)" }}>
          {title}
        </div>
        <div style={{ fontSize: 30, color: "rgba(255,255,255,0.9)", marginBottom: 20, fontWeight: 600 }}>{sub}</div>
